...
async function createCombinedImages(inputFolder, numCameras, numImagesPerCamera, finalWidth, finalHeight) {
    ...
    for (let i = 1; i <= numImagesPerCamera; i++) {
        ...
        for (let j = 1; j <= numCameras; j++) {
            const filePath = path.join(inputFolder, `cam${i}-img${j}.jpg`);
...
async function createSegment(inputFolder, numCameras, numImagesPerCamera, finalWidth, finalHeight, segmentIndex) {
    ...
    console.log("combinedImages.length=====================",combinedImages.length);
    const start = segmentIndex * 10;
    ...
    const audioFile = audioFiles[segmentIndex];
    if (!audioFile) {
        throw new Error(`Audio file not found for segment ${segmentIndex}`);
    }

    // Use ffmpeg to generate .ts segment directly from images and audio
    await new Promise((resolve, reject) => {
        const ffmpegCommand = ffmpeg()
            ...
            .outputOptions([
                ...
                '-hls_list_size 0',
                '-start_number 0',
                '-hls_segment_filename', path.join(segmentFolder, `segment_${segmentIndex}.ts`)
            ])
            .output(path.join(segmentFolder, `segment_${segmentIndex}.ts`)) // Required for HLS
    ...
    console.log(`TS segment ${segmentIndex} created and saved in: ${segmentFolder}`);
}
app.get('/manifest', async (req, res) => {
    ...
    for (let i = 0; i < audioFiles.length; i++) {
        ...
        manifestContent.push(`segment_${i}`);
    }
...
// API endpoint to serve a specific segment file
app.get('/:index', async (req, res) => {
    const inputFolder = 'D:\\HLS-test1';
    const numCameras = 6;
    const numImagesPerCamera = 500;
    const finalWidth = 640;
    const finalHeight = 480;
    const segmentIndex = parseInt(req.params.index.replace('segment_', ''), 10);

    try {
        const segmentPath = path.join(inputFolder, 'segments', `segment_${segmentIndex}.ts`);
        if (!fs.existsSync(segmentPath)) {
           await createSegment(inputFolder, numCameras, numImagesPerCamera, finalWidth, finalHeight, segmentIndex);
        }
