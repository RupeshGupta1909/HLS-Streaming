...
app.use('/segments', express.static(path.join('D:\\HLS-test', 'segments')));
// API endpoint to serve the manifest file
// app.get('/manifest', (req, res) => {
//     const manifestPath = path.join('D:\\HLS-test', 'segments', 'manifest.m3u8'); // Adjust this path as needed
...
app.get('/manifest', async (req, res) => {
    ...
    for (let i = 0; i < audioFiles.length; i++) {
        manifestContent.push(`#EXTINF:${segmentDuration.toFixed(1)},`);
        manifestContent.push(`segment_${i}.ts`); // Reference to the segment file
    }
